...
type editCommentFormData = z.infer<ReturnType<typeof editCommentFormSchemaBuilder>>;
...
interface EditCommentModalProps {
  ...
  handleEditComment: (newCommentContent: string) => void
}

export function EditCommentModal({ commentId, defaultCommentContentValue, isOpen, handleToggleOpen, handleEditComment }: EditCommentModalProps) {
  const schema = editCommentFormSchemaBuilder(defaultCommentContentValue)

  const { formState: { errors, isSubmitting }, handleSubmit, register } = useForm<editCommentFormData>({
